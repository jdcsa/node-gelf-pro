...
adapter._createSocket = function () {
  return dgram.createSocket({ type: this.options.protocol, sendBufferSize: this.options.bufferSize, lookup: this.options.customLookup });
};
...
adapter.send = function (message, callback) {
  ...
  self._socket = self._socket || self._createSocket()
